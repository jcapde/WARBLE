...
def lowerbound(tn, ln, wnm, a_pi, pi_, en_,mk_, m_mu, betamuk_,beta_mu, nuk_, nu_Delta, Wk_, W_Delta,mtauk_, m_tau, betatauk_, beta_tau, ak_, a_lambda, bk_, b_lambda, a_theta, thetak_, znm_, a_phi, phik_):
    ...
    Elogphi_ = dirichlet_expectation(phik_)
    ...
    for n in range(N):
        ...
        E3 += 1./2*np.dot(en_[n,:],(logDeltak -2.*np.log(2*math.pi) - nuk_*product - 2./betamuk_).T)
        E6 += 1./2*np.dot(en_[n,:],loglambdak-np.log(2.*math.pi)-1./betatauk_-ak_/bk_*(tn[n]-mtauk_)**2)
        E9 += np.dot(en_[n, :], np.dot(znm_[n, wordids, :]*CNTS, Elogtheta_.T).sum(axis=0))
        H9 += -np.sum(znm_[n, wordids,:]*log_(znm_[n, wordids, :])*CNTS)
        E10 += np.sum(znm_[n, wordids,:]*Elogphi_[:, wordids].T*CNTS)
    ...
    H7 = np.sum((1./2*(1. + np.log(2.*math.pi) - np.log(betatauk_*ak_/bk_))))
    ...
    E12 =  np.sum([-log_beta_function(arr_a_phi) + np.dot((arr_a_phi-np.ones(M)),Elogphi_[t,:])  for t in xrange(T)])
    H12 =  np.sum([log_beta_function(phik_[t,:]) - np.dot((phik_[t,:]-np.ones(M)),Elogphi_[t,:]) for t in xrange(T)])


    return E1 + E2 + E3 + E4 + E5 + E6 + E7 + E8 + E9 + E10 + E11 + E12 + H1 + H2 + H4 + H5 + H7 + H8 + H9 + H11 + H12


def inference_joint(It, tn, ln, wnm, a_pi, T, K, m_mu, beta_mu, W_Delta, nu_Delta, m_tau, beta_tau, a_lambda, b_lambda, a_theta, a_phi):
    N, M = wnm.shape
    ...
    phik_ = a_phi + np.sum(znm_*CNTS, axis=0).T
    ...
    it = 0
    lwbound_old = -1000.
    lwbound = lowerbound(tn, ln, wnm, a_pi, pi_, en_,mk_, m_mu, betamuk_,beta_mu, nuk_, nu_Delta, Wk_, W_Delta,mtauk_, m_tau, betatauk_, beta_tau, ak_, a_lambda, bk_, b_lambda, a_theta, thetak_, znm_, a_phi, phik_)
    ...
    while it<It and abs((lwbound-lwbound_old)/lwbound)>1e-4:
        ...
        for n in xrange(N):
            ...
            aux2 = np.exp(Elogphi_[:, wordids].T + np.tile(np.dot(en_[n, :], Elogtheta_), (len(wordids),1)))
